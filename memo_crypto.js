...
function loadJson(secretKey){ 
    ...
    let memoList = JSON.parse(JSON.parse(JSON.stringify(descryptedText)))
    ...
    while(true){
        ...
        switch(selectMenu){
            ...
            case 2: // 사용자가 "조회"를 선택했을 때
                ...
                console.log(`after loadJson, memoListW: ${memoListS}`)
            ...
            case 5: // 비밀번호 변경
                secretKey = loadSecretKey()
                const memoListPWD = loadJson(secretKey)
                console.log("?? 새로운 비밀번호를 입력하세요")
                secretKey = readlineSyncModule.question(">> ")
                saveSecretKey(secretKey)
                saveJson(memoListPWD, secretKey, "비밀번호가 변경")
                break
