...
while (true){
    ...
    switch (userSelect){
        case 1: // 사용자가 "작성"을 선택한 경우
            tmp = readlineSyncModule.question("제목: ")
        ...
        if(memoTitle.length === 0 && memoContent.length === 0){
            ...
            tmp = readlineSyncModule.question("새로운 제목: ")
            if(tmp === ""){ // 입력한 문자열이 없을 때 예외처리
                console.log("빈 문자열이 입력되었습니다.")
                break
            }
            memoTitle[select] = tmp

            tmp = readlineSyncModule.question("새로운 내용: ")
            if(tmp === ""){ // 입력한 문자열이 없을 때 예외처리
                console.log("빈 문자열이 입력되었습니다.")
                memoTitle.pop()
                break
            }
            memoContent[select] = tmp
            console.log("저장되었습니다.") // 사용자가 진행을 알 수 있도록 수행 완료 표시
            break
        }
